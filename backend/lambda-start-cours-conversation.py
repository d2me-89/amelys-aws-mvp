"""
Lambda: amelys-start-cours-conversation (VERSION MISE À JOUR)
Rôle: Démarrer ou récupérer une conversation (cours, exercices, binôme, contrôle, session-libre)
Auteur: Amélys Platform

CHANGEMENT MAJEUR:
- Vérifie d'abord si une conversation active existe déjà pour ce contenu
- Si oui: retourne la conversation existante
- Si non: crée une nouvelle conversation

Cela permet de persister les conversations même en changeant de navigateur.
"""
...
def find_existing_conversation(user_id, content_key, content_type):
    """
    Cherche une conversation active existante pour ce userId + contentKey + type
    
    Returns:
        dict ou None: La conversation si elle existe, None sinon
    """
    conversations_table = dynamodb.Table(TABLE_CONVERSATIONS)
    
    try:
        # Query pour récupérer toutes les conversations de cet utilisateur
        ...
        # Filtrer pour trouver une conversation correspondant au contenu
        for conv in response['Items']:
            if (conv.get('contentKey') == content_key and 
                conv.get('type') == content_type and
                conv.get('status') == 'active'):
                print(f"[FIND] Conversation existante trouvée: {conv['conversationId']}")
                return conv
        
        return None
        
    except Exception as e:
        print(f"[FIND] Erreur lors de la recherche: {str(e)}")
        return None


def lambda_handler(event, context):
    """
    Lambda handler pour démarrer ou récupérer une conversation
    
    NOUVELLE LOGIQUE:
    1. Vérifier si une conversation existe déjà pour ce contenu
    2. Si oui: retourner la conversation existante (sans appeler Claude)
    3. Si non: créer une nouvelle conversation et appeler Claude
    ...
    Output:
    {
        ...
        "isExisting": true/false  // NOUVEAU: indique si c'est une conversation existante
    }
    """
    ...
    try:
        ...
        existing_conv = find_existing_conversation(user_id, content_key, content_type)
        
        if existing_conv:
            print(f"[EXISTING] Conversation trouvée: {existing_conv['conversationId']}")
            ...
            # Charger les données du chapitre pour les infos
            ...
            content_data = content_response.get('Item', {})
            
            return success_response({
                'conversationId': existing_conv['conversationId'],
                'message': last_assistant_msg or "Conversation reprise.",
                'isExisting': True,  # NOUVEAU FLAG
                'messageCount': existing_conv.get('messageCount', 0),
                'contentInfo': {
        ...
        # ========================================
        # ÉTAPE 3 : PAS DE CONVERSATION EXISTANTE → CRÉER UNE NOUVELLE
        # ========================================
        
        print(f"[NEW] Aucune conversation existante, création d'une nouvelle")
        
        # ÉTAPE 3A : CHARGER LE TEMPLATE DE PROMPT
        ...
        # ÉTAPE 3B : CHARGER LES DONNÉES DU CHAPITRE
        content_data_table = dynamodb.Table(TABLE_CONTENT_DATA)
        ...
        # ÉTAPE 3C : CONSTRUIRE LE PROMPT COMPLET
        ...
        # ÉTAPE 3D : APPELER CLAUDE VIA BEDROCK
        ...
        # ÉTAPE 3E : SAUVEGARDER LA CONVERSATION
        ...
        # ÉTAPE 3F : RETOURNER LA RÉPONSE
        return success_response({
            'conversationId': conversation_id,
            'message': claude_message,
            'isExisting': False,  # NOUVEAU FLAG
            'contentInfo': {
                'niveau': fix_encoding(content_data.get('niveau', '')),
                'matiere': fix_encoding(content_data.get('matiere', '')),
                'chapitreTitre': fix_encoding(content_data.get('chapitreTitre', '')),
